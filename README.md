...
- **稽核軌跡**：每次批次掃描／遠端啟用記錄一筆 append-only 軌跡（操作者／時間／主機／結果），供合規追溯。
- **遠端啟用 OpenSSH**：對尚未開 SSH 的目標 PC，於「啟用 OpenSSH」分頁遠端（單台/批次）安裝並啟用 OpenSSH Server（agentless，免 WinRM；**僅在 Windows 管理機上有效**）。
...
| POST | `/api/ssh-enable` | 遠端啟用 OpenSSH（body：`targets[], concurrency?, operator?, allowedSource?`）→ `202`，背景並行（**僅 Windows 後端有效**） |
| GET | `/api/ssh-enable` | 列出啟用作業摘要 |
| GET | `/api/ssh-enable/:id` | 啟用進度／結果（**不含密碼**） |
