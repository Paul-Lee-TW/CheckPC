...
/**
 * 批次掃描表單：共用帳密 + 主機清單挑選 + 貼上主機 + 並行數 + 操作者。
 * onStarted(batchId) 於成功建立批次後呼叫。
 */
export function BatchScanForm({ onStarted }) {
  ...
  const buildTargets = () => {
    const fromInv = inventory
      ...
      .map((i) => ({ host: i.host, port: i.port || 22 }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-yellow-800 text-xs">
        ⚠️ 批次掃描會以「共用帳密」連線到多台主機，請僅在受信任的內網使用。密碼僅用於本次掃描、不會儲存或寫入軌跡。
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1">共用帳號</label>
          <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
            placeholder="Windows 使用者帳號" required className={inputCls} />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">共用密碼</label>
      ...
      <button type="submit" disabled={submitting}
        ...
        {submitting ? '建立批次中...' : `開始批次掃描（${targetCount} 台）`}
      </button>
    </form>
  );
}
